...
// -----------------------------------------------------------------------------------
{
	NSInteger row = [preferences integerForKey:@"useInches"];
	if (row == 0)
	{
...
// -----------------------------------------------------------------------------------
{
	...
	if (0 != [preferences integerForKey:@"useCaption"])
		[captionCheckbox setIntegerValue:[preferences integerForKey:@"useCaption"]];
...
// -----------------------------------------------------------------------------------
{
    ...
    if (returnCode == NSOKButton)
    {
        if ([[selected title] isEqualToString:@"TIFF"])
        {
            ...
            sized = [[[NSImage alloc] initWithData:[original TIFFRepresentation]] setDPI:[saveDPIText floatValue]];
